    ...
    <div className="w-80 shrink-0 border-r border-border overflow-y-auto">
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-3">
          <Label className="text-sm font-medium">{t('pages:changelog.changelogSource')}</Label>
          <RadioGroup
            value={sourceMode}
        ...
        {sourceMode === 'git-history' && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm">{t('pages:changelog.gitHistoryOptions')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {/* History Type */}
              <div className="space-y-2">
                <Label className="text-xs">{t('pages:changelog.historyType')}</Label>
                <Select
                  ...
                  <SelectContent>
                    <SelectItem value="since-version">
                      <div className="flex items-center gap-2">
                        ...
                        {t('pages:changelog.sinceVersion')}
                      </div>
                    </SelectItem>
                    <SelectItem value="recent">
                      <div className="flex items-center gap-2">
                        <History className="h-3 w-3" />
                        {t('pages:changelog.recentCommits')}
                      </div>
                    </SelectItem>
                    <SelectItem value="since-date">
                      <div className="flex items-center gap-2">
                        <Calendar className="h-3 w-3" />
                        {t('pages:changelog.sinceDate')}
                      </div>
                    </SelectItem>
                    <SelectItem value="tag-range">
                      <div className="flex items-center gap-2">
                        <Tag className="h-3 w-3" />
                        {t('pages:changelog.tagRange')}
                      </div>
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              {gitHistoryType === 'recent' && (
                <div className="space-y-2">
                  <Label className="text-xs">{t('pages:changelog.numberOfCommits')}</Label>
                  <Input
                    type="number"
                    min={1}
                    max={500}
              ...
              {gitHistoryType === 'since-date' && (
                <div className="space-y-2">
                  <Label className="text-xs">{t('pages:changelog.sinceDate')}</Label>
                  <Input
                    type="date"
              ...
              {gitHistoryType === 'tag-range' && (
                <>
                  <div className="space-y-2">
                    <Label className="text-xs">{t('pages:changelog.fromTag')}</Label>
                    <Select value={gitHistoryFromTag} onValueChange={onGitHistoryFromTagChange}>
                      <SelectTrigger>
                        <SelectValue placeholder={t('pages:changelog.tags') + '...'} />
                      </SelectTrigger>
                      <SelectContent>
                        {tags.map((tag) => (
                  ...
                  <div className="space-y-2">
                    <Label className="text-xs">{t('pages:changelog.toTag')}</Label>
                    <Select value={gitHistoryToTag || 'HEAD'} onValueChange={(v) => onGitHistoryToTagChange(v === 'HEAD' ? '' : v)}>
                      <SelectTrigger>
                        <SelectValue placeholder={t('pages:changelog.headLatest')} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="HEAD">{t('pages:changelog.headLatest')}</SelectItem>
                        {tags.map((tag) => (
                          <SelectItem key={tag.name} value={tag.name}>
                            {tag.name}
                          </SelectItem>
              ...
              {gitHistoryType === 'since-version' && (
                <div className="space-y-2">
                  <Label className="text-xs">{t('pages:changelog.lastVersion')}</Label>
                  <Select value={gitHistorySinceVersion} onValueChange={onGitHistorySinceVersionChange}>
                    <SelectTrigger>
                      <SelectValue placeholder={t('pages:changelog.version') + '...'} />
                    </SelectTrigger>
                    <SelectContent>
                      {tags.map((tag) => (
                        <SelectItem key={tag.name} value={tag.name}>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t('pages:changelog.allCommitsSinceVersion')}
                  </p>
                </div>
              )}
              ...
              <div className="flex items-center gap-2">
                ...
                <Label htmlFor="merge-commits" className="text-xs cursor-pointer">
                  {t('pages:changelog.includeMergeCommits')}
                </Label>
              </div>
              ...
              >
                {isLoadingCommits ? (
                  <>
                    ...
                    {t('pages:changelog.loadingCommits')}
                  </>
                ) : (
                  <>
                    <RefreshCw className="mr-2 h-4 w-4" />
                    {t('pages:changelog.loadCommits')}
                  </>
                )}
              </Button>
            </CardContent>
        ...
        {sourceMode === 'branch-diff' && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm">{t('pages:changelog.branchComparison')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label className="text-xs">{t('pages:changelog.baseBranch')}</Label>
                <Select value={baseBranch} onValueChange={onBaseBranchChange}>
                  <SelectTrigger>
                    <SelectValue placeholder={t('pages:changelog.baseBranch') + '...'} />
                  </SelectTrigger>
                  <SelectContent>
                      ...
                      <SelectItem key={branch.name} value={branch.name}>
                        <div className="flex items-center gap-2">
                          ...
                          {branch.name === defaultBranch && (
                            <Badge variant="outline" className="text-xs">{t('pages:changelog.default')}</Badge>
                          )}
                        </div>
                      </SelectItem>
                    ))}
                ...
                <p className="text-xs text-muted-foreground">
                  {t('pages:changelog.baseBranchDesc')}
                </p>
              </div>

              <div className="space-y-2">
                <Label className="text-xs">{t('pages:changelog.compareBranch')}</Label>
                <Select value={compareBranch} onValueChange={onCompareBranchChange}>
                  <SelectTrigger>
                    <SelectValue placeholder={t('pages:changelog.compareBranch') + '...'} />
                  </SelectTrigger>
                  <SelectContent>
                    {localBranches.map((branch) => (
                      <SelectItem key={branch.name} value={branch.name}>
                        <div className="flex items-center gap-2">
                          {branch.name}
                          {branch.isCurrent && (
                            <Badge variant="secondary" className="text-xs">{t('pages:changelog.current')}</Badge>
                          )}
                        </div>
                      </SelectItem>
                    ))}
                ...
                <p className="text-xs text-muted-foreground">
                  {t('pages:changelog.compareBranchDesc')}
                </p>
              </div>
              ...
              {baseBranch && compareBranch && baseBranch === compareBranch && (
                <div className="flex items-center gap-2 text-destructive text-xs">
                  ...
                  {t('pages:changelog.branchesMustBeDifferent')}
                </div>
              )}
