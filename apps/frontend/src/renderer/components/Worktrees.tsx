...
export function Worktrees({ projectId }: WorktreesProps) {
  const { t } = useTranslation(['pages', 'common', 'dialogs']);
  ...
  if (!selectedProject) {
    return (
      <div className="flex h-full items-center justify-center">
        <p className="text-muted-foreground">{t('pages:agentTools.selectProject')}</p>
      </div>
    );
  }
    ...
    <div className="flex h-full flex-col p-6">
      ...
      {!isLoading && worktrees.length === 0 && (
        <div className="flex h-full flex-col items-center justify-center text-center">
          ...
          <h3 className="text-lg font-semibold text-foreground">{t('pages:worktrees.noWorktrees')}</h3>
          <p className="text-sm text-muted-foreground mt-2 max-w-md">
            {t('pages:worktrees.description')}
          </p>
        </div>
      )}
                  ...
                  <CardContent className="pt-0">
                    ...
                    <div className="flex flex-wrap gap-4 text-sm mb-4">
                      <div className="flex items-center gap-1.5 text-muted-foreground">
                        ...
                        <span>{t('pages:worktrees.filesChanged', { count: worktree.filesChanged })}</span>
                      </div>
                      <div className="flex items-center gap-1.5 text-muted-foreground">
                        <ChevronRight className="h-3.5 w-3.5" />
                        <span>{t('pages:worktrees.commitsAhead', { count: worktree.commitCount })}</span>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      >
                        ...
                        {t('pages:worktrees.merge')} to {worktree.baseBranch}
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
                      ...
                      >
                        ...
                        {t('common:buttons.copy', 'Copy Path')}
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {t('pages:worktrees.mergeWorktree')}
            </DialogTitle>
            <DialogDescription>
              {t('pages:worktrees.mergeDescription')}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="py-4">
              <div className="rounded-lg bg-muted p-4 text-sm space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">{t('pages:worktrees.sourceBranch')}</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">{t('pages:worktrees.targetBranch')}</span>
                  <span className="font-mono">{selectedWorktree.baseBranch}</span>
                </div>
                <div className="border-t border-border pt-3 mt-3">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-muted-foreground">{t('pages:worktrees.changes')}</span>
                    <span>
                      {t('pages:worktrees.commitsAndFiles', { commits: selectedWorktree.commitCount, files: selectedWorktree.filesChanged })}
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className={`font-medium ${mergeResult.success ? 'text-success' : 'text-destructive'}`}>
                      {mergeResult.success ? t('pages:worktrees.mergeSuccessful') : t('pages:worktrees.mergeFailed')}
                    </p>
                    <p className="text-muted-foreground mt-1">{mergeResult.message}</p>
                    {mergeResult.conflictFiles && mergeResult.conflictFiles.length > 0 && (
                      <div className="mt-2">
                        <p className="text-xs font-medium">{t('pages:worktrees.conflictingFiles')}</p>
              ...
              >
                {isMerging ? (
                  <>
                    ...
                    {t('pages:worktrees.merging')}
                  </>
                ) : (
                  <>
                    <GitMerge className="h-4 w-4 mr-2" />
                    {t('pages:worktrees.merge')}
                  </>
                )}
              </Button>
            )}
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('pages:worktrees.deleteWorktree')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('pages:worktrees.deleteDescription')}
              {worktreeToDelete && (
                <span className="block mt-2 font-mono text-sm">
                  {worktreeToDelete.branch}
                </span>
              )}
              {t('pages:worktrees.cannotUndo')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
