...
export function ChatHistorySidebar({
  ...
  const formatDate = (date: Date) => {
    ...
    if (diffDays === 0) {
      return t('pages:chatHistory.today');
    } else if (diffDays === 1) {
      return t('pages:chatHistory.yesterday');
    } else if (diffDays < 7) {
      return t('pages:chatHistory.daysAgo', { count: diffDays });
    } else {
  ...
  return (
    <div className="flex h-full w-64 flex-col border-r border-border bg-muted/30">
      ...
      <div className="flex items-center justify-between border-b border-border px-3 py-3">
        <h3 className="text-sm font-medium text-foreground">{t('pages:chatHistory.title')}</h3>
        <Button
          variant="ghost"
          size="icon"
          className="h-7 w-7"
          onClick={onNewSession}
          title={t('pages:chatHistory.newConversation')}
        >
      ...
      <ScrollArea className="flex-1">
        {isLoading ? (
          ...
          <div className="px-3 py-8 text-center text-sm text-muted-foreground">
            {t('pages:chatHistory.noConversations')}
          </div>
        ) : (
          <div className="py-2">
      ...
      <AlertDialog open={!!deleteSessionId} onOpenChange={() => setDeleteSessionId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('pages:chatHistory.deleteTitle')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('pages:chatHistory.deleteDescription')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t('pages:chatHistory.cancel')}</AlertDialogCancel>
            <AlertDialogAction onClick={handleDelete}>{t('pages:chatHistory.delete')}</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
      ...
      <div className="flex items-center gap-1.5 pr-7">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5">
            {session.messageCount} {session.messageCount !== 1 ? t('pages:chatHistory.messagesPlural') : t('pages:chatHistory.messages')}
          </p>
        </div>
      </div>
